...
const parqueoslleno = (libres , vacias) => {
  const boton = document.querySelector('form button');
  if(vacias.length === 0){
    boton.disabled = true
    input.disabled = true
    libres.innerHTML = "Parqueo Lleno";
    libres.style.color = "red";
  } else{
    libres.innerHTML = `Tienes ${vacias.length} parqueos libres`;
    libres.style.color = "green";
...
const actualizarMensaje = () => {
  ...
  const Libres = document.querySelector("body h3");

  // Agarrar todos las celdas vacias
  const Vacias = document.querySelectorAll("td:empty");

  // Agregar la cantidad de celdas vacias a `espaciosLibres`
  Libres.innerText = Vacias.length;
  parqueoslleno(Libres, Vacias)
};
